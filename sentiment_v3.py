...
def preprocess_text(text):
    """Cleans text while preserving sentence structure for VADER."""
    text = text.lower()
    # Remove stage directions
    text = re.sub(r'\[[^\]]+\]', '', text)
    # Remove character names at start of lines
    text = re.sub(r'^\w+:', '', text, flags=re.MULTILINE)
    # Split into sentences
    sentences = re.split(r'[.!?]', text)
    return [s.strip() for s in sentences if s.strip()]

def analyze_vader(sentences):
    """Uses VADER to get a compound score between -1 and 1."""
    print(f"{'SENTIMENT':<12} | {'SCORE':<8} | {'SENTENCE'}")
    print("-" * 60)
    ...
    for sentence in sentences[:15]:
        ...
        print(f"{label:<12} | {score:>8.2f} | {sentence[:50]}...")
...
def run_analysis(text_to_analyze):
    ...
    for sentence in sentences[:20]:
        ...
        lexicon_sentiments.append(sentiment)
        print(f"[{sentiment}] {sentence[:60]}...")
        if pos: print(f"  (+) Matched: {', '.join(pos)}")
        if neg: print(f"  (-) Matched: {', '.join(neg)}")
    
    display_summary(lexicon_sentiments)

    print("\n" + "-" * 30)
    print("VADER (NLTK) Results")
    print("-" * 30)
    analyze_vader(sentences)
